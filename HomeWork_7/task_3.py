...
class PublicTransport:
    ...
    def info(self):
        """
        return: Вывод информация об автомобиле
        """
        car_info = f"Brand: {self.brand}\nColor: {self.color}\nYear: {self.year}\nEngine Power: {self._engine_power}"
        return car_info
...
class Bus(PublicTransport):
    ...
    def park(self):
        """
        return: Вывод значения park
        """
        return self.__park

    @park.setter
    def park(self, value):
        """ Проверка номера парка внутри диапозона """
        if 1000 <= value <= 9999:
            self.__park = value
        else:
...
class Tram(PublicTransport):
    ...
    def how_long(self):
        """
        Вычисление времени прохождения маршрута
        return: Время
        """
        path_time = self.max_speed / (4 * self.path)
        return path_time
